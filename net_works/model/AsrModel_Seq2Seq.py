...
class Seq2Seq(nn.Module):
    ...
    def forward(self, data, eval=False):
        ...
        if not eval:
            out = self.decoder(targets, enc_y, c_hid)
        else:
            out, logp = self.greedy_decode(enc_y, c_hid)
            # out, logp = self.beam_search(enc_y)
        return out

    def greedy_decode(self, enc_y, c_hid):
        ...
        while STOP is False:  # 0 represent the last word [end].
            output, c_hid, ax, sx = self.decoder._step(inputs, c_hid, enc_y, ax, sx)
...
class Decoder(nn.Module):
    ...
    def forward(self, target, enc_y, c_hid):  # targets, enc_y, c_hid
        '''
        `target`: (batch, length)
